...
layui.define(["form","jquery"],function(exports){
    ...
    Address.prototype.provinces = function() {
        ...
        // $.get("address.txt", function (data) {
            for (var i = 0; i < datas.length; i++) {
                proHtml += '<option value="' + datas[i].name + '">' + datas[i].name + '</option>';
            }
            //初始化省数据
            $("select[name=org_province]").append(proHtml);
            form.render();
            form.on('select(org_province)', function (proData) {
                $("select[name=org_area]").html('<option value="">请选择县/区</option>');
                var value = proData.value;
                if (value != "" || value != NULL) {
                    that.citys(datas[$(this).index()].cityList);
                } else {
                    $("select[name=org_city]").attr("disabled", "disabled");
                }
            });
        // })
    }
    ...
    Address.prototype.citys = function(citys) {
        ...
        $("select[name=org_city]").html(cityHtml).removeAttr("disabled");
        form.render();
        form.on('select(org_city)', function (cityData) {
            var value = cityData.value;
            if (value != "" || value != NULL) {
                that.areas(citys[$(this).index()].areaList);
            } else {
                $("select[name=org_area]").attr("disabled", "disabled");
            }
        });
    }
    ...
    Address.prototype.areas = function(areas) {
        var areaHtml = '<option value="">请选择县/区</option>';
            for (var i = 0; i < areas.length; i++) {
                    ...
                    areaHtml += '<option value="' + areas[i] + '">' + areas[i] + '</option>';

            }

        $("select[name=org_area]").html(areaHtml).removeAttr("disabled");
        form.render();
    }
